...
//
// Window corners and shell margins share niri-style.json with the compositor
// theme generator. Shell margins use half the window gap to retain Omarchy's
// compact spacing at the edge of the screen.
//
...
QtObject {
  ...
  property FileView geometryFile: FileView {
    path: Quickshell.env("HOME") + "/.config/omarchy/niri-style.json"
    watchChanges: true
    printErrors: false
    ...
    onLoaded: {
      try {
        ...
        if (typeof geometry.radius === "number" && isFinite(geometry.radius) && geometry.radius >= 0 && geometry.radius <= 100) root.cornerRadius = geometry.radius
        if (typeof geometry.gaps === "number" && isFinite(geometry.gaps) && geometry.gaps >= 0 && geometry.gaps <= 100) root.gapsOut = Math.round(geometry.gaps / 2)
      } catch (error) { console.warn("Invalid niri style:", error) }
    }
  }
